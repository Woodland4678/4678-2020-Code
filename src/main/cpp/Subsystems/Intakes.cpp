...
rightRoller.reset(new rev::CANSparkMax(8, rev::CANSparkMax::MotorType::kBrushless));
    ...
    intakeLeftSensor.reset(new frc::DigitalInput(3));
    AddChild("intakeLeftSensor", intakeLeftSensor);

    intakeRightSensor.reset(new frc::DigitalInput(2));
    AddChild("intakeRightSensor", intakeRightSensor);
...
void Intakes::Periodic() {
    ...
    frc::SmartDashboard::PutBoolean("Intake Right",getRightSensor());
    frc::SmartDashboard::PutNumber("Middle Amp", middleRoller->GetOutputCurrent());
    frc::SmartDashboard::PutNumber("Left Amp", leftRoller->GetOutputCurrent());
    frc::SmartDashboard::PutNumber("Right Amp", rightRoller->GetOutputCurrent());
    ...
    if(m_deployed){
        if((getRightSensor() & getLeftSensor()) == true){
            slowRight = true;
            setRight(-0.2);
        }
        else{
        ...
        if(magazineSensorHigh->Get() == true){
            ...
            stopMag();
        if((magazineSensorHigh->Get() == false)&&(magazineSensorLow->Get() == false)){
            shiftCells(true);
            stopMag();
